...
func TestURLRequest(t *testing.T) {
	tests := []struct {
		...
		{"GET method, non-empty url, single option", "GET", "http://fake.com/", []FuncOption{Limit(15)}, httptest.NewRequest("GET", "http://fake.com/limit%2015%3B%20", nil), nil},
		...
		{"POST method, non-empty url, single option", "POST", "http://fake.com/", []FuncOption{Limit(15)}, httptest.NewRequest("POST", "http://fake.com/limit%2015%3B%20", nil), nil},
		...
		{"Empty method, non-empty url, single option", "", "http://fake.com/", []FuncOption{Limit(15)}, httptest.NewRequest("GET", "http://fake.com/limit%2015%3B%20", nil), nil},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			req, err := URLRequest(test.method, test.url, test.opts...)
...
func TestBodyRequest(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			req, err := BodyRequest(test.method, test.url, test.opts...)
			if !reflect.DeepEqual(errors.Cause(err), test.wantErr) {
				t.Errorf("got: <%v>, want: <%v>", err, test.wantErr)
			}
